...
public class Amalgamate implements AmalgamateInt, Serializable{
	...
	//Iterates through each received file from the workers, generating one larger output file of all the hashes
	/* (non-Javadoc)
	...
	public boolean amalgamateFile() {
	    ...
	    for(File result : files){
 		...
 		/// framework - DO NOT REMOVE
	    	...
	    	int fileSize = das.size();
	    	for(int i = 0; i <= fileSize - 1; i++){
	    	    dio.appendItem(das.readValue(i));
	    	}
